...
for popn = {'Blind','Control'}
    ...
    for sub = 1:length(dir_deriv)
        ...
        acc{pop}(:,:,sub_ii) = mean(auc_mean,2); % comp x time x sub
        sub_ii = sub_ii+1;
...
timewin(end) = [];
tp = 49:50;
vec1 = squeeze(mean(acc{1}(:,tp,:),2))-0.5;
vec2 = squeeze(mean(acc{2}(:,tp,:),2))-0.5;
permnum = 1000;
thr = 0.05;
[src,pval] = fn_signperm_src(vec1,vec2,permnum,thr);

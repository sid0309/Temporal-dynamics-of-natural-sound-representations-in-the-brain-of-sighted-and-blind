...
for trig = 1:24
    epochs{trig} = zeros(size(preproc{1},1),size(preproc{1},2),1); % create data structure
end

for tr = 1:length(preproc)
    epochs{trinfo(tr)}(:,:,size(epochs{trinfo(tr)},3)+1) = preproc{tr}; % ep_ds_bl_tr{trigno}(ch x tp x trials)
end
